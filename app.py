...
def handle_message(event):
    try:
        ...
        if user_message.startswith("monitor"):
            ...
            skus = [sku.strip() for sku in skus.replace(",", " ").split()]  # รองรับทั้ง "," และ ", " และ " ,"
        ...
        elif all(sku.strip().isalnum() for sku in user_message.replace(",", " ").split()):
...
def handle_stock_inquiry(event):
    user_id = event.source.user_id
    product_codes = event.message.text.replace(",", " ").split()
